...
namespace ias
{
    ...
    bool Tissue::calculateInteractingElements(double eps)
    {
        using namespace std;
        ...
        {
            int glo_1 = _inters[_myPart][inte].first;
            int glo_2 = _inters[_myPart][inte].second;

            RCP<Cell> cell_1 = GetCell(glo_1);
            RCP<Cell> cell_2 = GetCell(glo_2);
            
//             //Compute bounding boxes
//             tensor<double,2> box1 = cell_1->getBoundingBox(eps);
//             tensor<double,2> box2 = cell_2->getBoundingBox(eps);

//             //Get elements of each cell in the opposite box
//             std::vector<int> elems_inte1 = cell_1->getElementsInBox(box2);
//             std::vector<int> elems_inte2 = cell_2->getElementsInBox(box1);
            
//             vector<pair<int,int>> elems_inte;
//             for(int e: elems_inte1)
//             {
//                 int eNN_1 = cell_1->_bfs->getNumberOfNeighbours(e);

//                 #pragma omp parallel for
//                 for(size_t idx = 0; idx < elems_inte2.size(); idx++)
//                 {
//                     int g = elems_inte2[idx];

//                     int eNN_2 = cell_2->_bfs->getNumberOfNeighbours(g);

//                     vector<double> distances(eNN_1*eNN_2);

//                     for(int i=0; i < eNN_1; i++)
//                     {
//                         int ii = cell_1->_bfs->getNeighbours(e)[i];
//                         tensor<double,1> x1 = cell_1->_nodeFields(ii,range(0,2));
//                         for(int j = 0; j < eNN_2; j++)
//                         {
//                             int jj = cell_2->_bfs->getNeighbours(g)[j];
//                             tensor<double,1> x2 = cell_2->_nodeFields(jj,range(0,2));

//                             distances[eNN_2*i+j] = sqrt((x1-x2)*(x1-x2));
//                         }
//                     }
...
//             {
// //                cout << _elems_inte[inte].size() << " " << elems_inte.size() << endl;
// //                if(_elems_inte[inte].size() == elems_inte.size())
// //                {
// //                    for(size_t i = 0; i < elems_inte.size(); i++ )
// //                    {
// //                        if(elems_inte[i]!=_elems_inte[inte][i])
// //                            cout << elems_inte[i].first << " " << elems_inte[i].second << " " << _elems_inte[inte][i].first << " " << _elems_inte[inte][i].second << endl;
// //                    }
// //                }
...
//             }
            ...
            for(int e = 0; e < cell_2->getNumberOfElements(); e++)
            {
                ...
                for(int k = 0; k < 3; k++)
                {
                    auto vbfs = cell_2->_bfs->computeBasisFunctions(pts[k],e);
                    tensor<double,1> bfs(vbfs[0].data(),eNN_2);
            ...
            for(int e = 0; e < cell_1->getNumberOfElements(); e++)
            {
                ...
                for(int k = 0; k < 3; k++)
                {
                    auto vbfs = cell_1->_bfs->computeBasisFunctions(pts[k],e);
                    tensor<double,1> bfs(vbfs[0].data(),eNN_1);
                    tensor<double,1> x = bfs * nborCoords;
                    ...
                    for(int g = 0; g < n; g++)
                    {
                        int elemId = result->GetId(g)/3;
                        elems_inte.push_back({e,elemId});   
                    }        
                }
            }

            sort(elems_inte.begin(),elems_inte.end());
            elems_inte.erase(unique( elems_inte.begin(), elems_inte.end() ), elems_inte.end() );
            if(_elems_inte[inte] != elems_inte)
            {
                _elems_inte[inte] = elems_inte;
                change = 1;
